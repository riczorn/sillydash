...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.openDrillDown = function (d) {
        ...
        fetch(detailUrl + '?date=' + d.date + '&type=' + d.type)
            ...
            .then(details => {
                ...
                var html = '<table style="width:100%;text-align:left"><thead><tr><th>Domain</th><th style="text-align:right">Size</th></tr></thead><tbody>';
                details.forEach(item => {
                    html += '<tr><td>' + item.domain + '</td><td style="text-align:right">' + formattedBytes(item.size) + '</td></tr>';
                });
                html += '</tbody></table>';
    ...
    const pieContainer = document.getElementById('accountsPieChartContainer');
    if (pieContainer) {
        const pieUrl = pieContainer.dataset.distributionUrl;

        function loadAccountsDistribution() {
            pieContainer.innerHTML = '<p style="text-align:center;padding:2rem">Loading…</p>';
            fetch(pieUrl)
                .then(r => r.json())
                .then(data => renderPieChart(data))
                .catch(err => {
                    console.error('Pie chart error:', err);
                    pieContainer.innerHTML = '<p style="text-align:center;color:red">Error loading distribution data</p>';
                });
        }
        ...
        function renderPieChart(data) {
            ...
            const w = pieContainer.clientWidth;
            const h = pieContainer.clientHeight;
            ...
            // Add tooltips
            const tooltip = d3.select(pieContainer).append('div')
            ...
            arcs.append('path')
                .attr('d', arc)
                .attr('fill', (d, i) => d.data.is_others ? '#999' : color(i))
                .style('opacity', 0.8)
                .on('mouseover', function (event, d) {
                    d3.select(this).style('opacity', 1);
                    d3.select(this).style('cursor', d.data.is_others ? 'default' : 'pointer');
                    tooltip.transition().duration(200).style('opacity', .9);
                    tooltip.html('<strong>' + d.data.label + '</strong><br/>' + formattedBytes(d.data.value))
                ...
                .on('click', function (event, d) {
                    if (!d.data.is_others) {
                        window.location.href = '?account=' + encodeURIComponent(d.data.label);
                    }
                });

            // Legend
            const legendG = d3.select(pieContainer).select('svg').append('g')
                .attr('transform', `translate(${width + margin.left}, 40)`);
            ...
            legend.append('rect')
                .attr('width', 10)
                .attr('height', 10)
                .style('fill', (d, i) => d.is_others ? '#999' : color(i));
        ...
        loadAccountsDistribution();
        window.addEventListener('resize', loadAccountsDistribution);
    }
});
