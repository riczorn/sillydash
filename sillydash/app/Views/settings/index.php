...
<div class="card" style="max-width: 800px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
        ...
        <button type="button" id="runCronBtn" class="btn-primary"
            style="width: auto; margin: 0; background-color: #4CAF50; padding: 0.5rem 1rem; font-size: 0.9rem;">
            ▶ Run Cron Now
        </button>
    </div>
    ...
    <?php if (session()->getFlashdata('message')): ?>
        <div class="result-display success"
            style="border-left: 4px solid var(--secondary-color); color: var(--secondary-color);">
    ...
    <form action="<?= site_url('settings/save') ?>" method="post">
        ...
        <div class="form-group">
            ...
            <small class="form-hint">Used to authenticate cron requests: <code>/cron?token=YOUR_TOKEN</code></small>
        </div>
        ...
        <div class="form-group" style="margin-top: 0.5rem;">
            <button type="button" id="testSmtpBtn" <?= !$hasEmail ? 'disabled' : '' ?>
                style="padding: 0.5rem 1rem; background: <?= $hasEmail ? '#444' : '#333' ?>; color: <?= $hasEmail ? 'var(--text-main)' : 'var(--text-muted)' ?>; border: 1px solid var(--border-color); border-radius: 4px; cursor: <?= $hasEmail ? 'pointer' : 'not-allowed' ?>; font-size: 0.9rem; transition: opacity 0.2s;">
                ✉ Test Configuration
            </button>
            <?php if (!$hasEmail): ?>
                <small class="form-hint" style="color: #cf6679;">Your user account has no email address — set one in Users
                    to enable testing.</small>
            <?php else: ?>
                <small class="form-hint">Sends a test email to <strong><?= esc($userEmail) ?></strong></small>
            <?php endif; ?>
            <div id="testSmtpResult"
                style="margin-top: 0.5rem; display: none; padding: 0.75rem; border-radius: 4px; font-size: 0.9rem;">
            </div>
        </div>

        <h2>Database <small style="font-weight:normal; font-size:0.7em; color:var(--text-muted);">(read only)</small>
        </h2>
        <div class="form-group">
            <label>Host</label>
...
<script>
    ...
    if (cronBtn) {
        cronBtn.addEventListener('click', functio n() {
            if(!cronToken) {
